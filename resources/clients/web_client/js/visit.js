...
function ViewerMethods(state) 
{
    ...
    {
        viewerState.set(0,"RPCType",visitRPC["AddWindowRPC"]);
        viewerState.notify(0);
    }
    ...
    this.AddOperator = function(name) {
        var id = getEnabledId("operator",name);

        if(id < 0) return;

        this.SetActiveWindow(windowId);
        viewerState.set(0,"RPCType",visitRPC["AddOperatorRPC"]);
        viewerState.set(0,"operatorType",id);
        viewerState.notify(0);
    }

    this.SetOperatorOptions = function(name) {
        var id = getEnabledId("operator",name);

        if(id < 0) return;

        this.SetActiveWindow(windowId);
        viewerState.set(0,"RPCType",visitRPC["SetOperatorOptionsRPC"]);
        viewerState.set(0,"operatorType",id);
        viewerState.notify(0);
    }


    this.RemoveOperator = function(index) {
        if(index < 0) return;

        this.SetActiveWindow(windowId);
        viewerState.set(0,"RPCType",visitRPC["RemoveOperatorRPC"]);
    ...
    this.RegisterNewWindow = function(windowId, type) {
    ...
    this.GetWindowIds = function() {
        var ids = [];

        var gattr = viewerState.getIndexFromTypename("GlobalAttributes");
        var array = viewerState.get(gattr,"windows");
